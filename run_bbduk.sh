...
rawdatalist=$(ls -d $RAWDATA_FOLDER/*_1.fastq.gz | awk '{print $NF}') #prints last column/field

for read in ${rawdatalist}
do

read2=${read%_1*}_2.fastq.gz
fname=$(basename ${read} | sed -e "s/_1.fastq.gz//") #strip extension
echo "Running bbduk on sample ${fname}"
echo "adapter.fa hardcoded - provide resource for final version"
