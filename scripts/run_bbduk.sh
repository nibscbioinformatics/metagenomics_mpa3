...
rawdatalist=$(ls -d $RAWDATA_FOLDER/*_R1_001.fastq.gz | awk '{print $NF}') # fastq extension format - if not '*_R*_001.fastq' modify
...
bbduk.sh \
 ...
 ktrim=r k=21 mink=10 hdist=2 qtrim=r \
 trimq=25 minlen=100 tpe tbo t=7 >>bbduk_trimmed_reads/bbduklog.txt 2>&1
